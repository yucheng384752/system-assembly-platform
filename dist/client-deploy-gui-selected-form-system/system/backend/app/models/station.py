...
class StationSchema(Base):
    ...
    item_fields: Mapped[list[dict[str, Any]] | None] = mapped_column(JSONB, nullable=True)
    unique_key_fields: Mapped[list[str]] = mapped_column(JSONB, nullable=False)
    csv_signature_columns: Mapped[list[str] | None] = mapped_column(JSONB, nullable=True)
    csv_filename_pattern: Mapped[str | None] = mapped_column(String(100), nullable=True)
    csv_field_mapping: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
...
class StationLink(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "tenant_id", "from_station_id", "to_station_id", name="uq_station_links_pair"
        ),
    )
    ...
    link_config: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, server_default="{}")
    sort_order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
