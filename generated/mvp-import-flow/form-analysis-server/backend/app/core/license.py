...
Machine binding:
  - machineFingerprint uses TPM EK certificate SHA-256 first, then /etc/machine-id or Windows MachineGuid.
...
def verify_license() -> LicenseResult:
    ...
    # Machine fingerprint binding (TPM EK first, machine-id fallback).
    ...
    if expected_fp:
        ...
        if current_fp is None:
            return LicenseResult(valid=False, reason="cannot read machine fingerprint")
...
def _get_machine_fingerprint() -> str | None:
    """Return TPM EK SHA-256 when available; otherwise machine-id/MachineGuid SHA-256."""
