...
class _PerCallProxy:
    """Creates a fresh FileValidationService per call to prevent shared mutable state across concurrent requests."""

    def validate_file(self, file_content: bytes, filename: str) -> dict[str, Any]:
        return FileValidationService().validate_file(file_content, filename)


file_validation_service = _PerCallProxy()
